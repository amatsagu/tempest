...
func main() {
	...
	log.Println("Creating new Qord client...")
	client := qord.NewClient()

	addr := os.Getenv("DISCORD_APP_ADDRESS")
	testServerID, err := api.StringToSnowflake(os.Getenv("DISCORD_TEST_SERVER_ID")) // Register example commands only to this guild.
	if err != nil {
	...
	err = client.SyncCommandsWithDiscord([]api.Snowflake{testServerID}, nil, false)
	if err != nil {
